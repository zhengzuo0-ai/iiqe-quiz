  ...
  return (
    <div className="animate-fade-in">
      <div className="gradient-banner rounded-2xl px-5 py-5 mb-5 text-white">
        <h2 className="font-display text-2xl font-semibold mb-1">错题本</h2>
      ...
      {activeErrors.length === 0 ? (
        <div className="text-center py-16">
          ...
          <p className="text-charcoal-light/50 text-sm">还没有错题，继续保持！</p>
        </div>
      ) : (
        <>
          <button
            onClick={onStartReview}
            className="w-full mb-4 py-3.5 text-white rounded-xl text-sm font-semibold btn-primary"
          >
                  ...
                  <div className="border-t border-cream-100">
                    {errors.map(err => (
                      <div key={err.id} className="border-b border-cream-100 last:border-b-0">
                        ...
                        >
                          <p className="text-sm text-charcoal-light leading-relaxed line-clamp-2">{err.question.question}</p>
                        ...
                        {expandedError === err.id && (
                          <div className="px-3.5 pb-3.5 animate-fade-in">
                            <div className="gradient-pink-purple-light rounded-lg p-3 mb-2">
                              <div className="text-xs text-charcoal-light/50 mb-2 font-medium">选项：</div>
                              {Object.entries(err.question.options).map(([k, v]) => (
                                <div key={k} className={`text-xs py-0.5 ${
                                  ...
                                  k === err.userAnswer ? 'text-coral-400' : 'text-charcoal-light/50'
                                }`}>
                                  {k}. {v} {k === err.correctAnswer ? '✓' : k === err.userAnswer ? '✗' : ''}
                                </div>
                              ))}
                            </div>
                            <p className="text-xs text-charcoal-light/60 leading-relaxed mb-2">{err.question.explanation}</p>
                            <button
